...
def main(argv=None):
	...
	unsd_csv = './input/csv/medicare_data.txt'
	unsd_data_frame = read_csv(unsd_csv, '\t')
	#unsd_data_frame = read_csv(unsd_csv)
	logging.info(msg[0].format(os.path.abspath(unsd_csv)))

	# Write drg_def to a .csv file.
	drg_def = extract_filtered_series(unsd_data_frame, 'drg_def')
	drg_def_csv = './output/drg_def.txt'
	write_series_to_csv(drg_def, drg_def_csv, '\t', False)
	logging.info(msg[1].format(os.path.abspath(drg_def_csv)))
