...
DROP TABLE IF EXISTS drg, provider_drg, provider_temp, referral_region, address_temp, provider, address, city_state_temp, city_state, t_medicare, t_medicare_1, f_medicare;
...
CREATE TABLE IF NOT EXISTS city_state_temp
  (
...
-- This should populate my tables with my unique values I pulled from the t_medicare_1 table. 

INSERT IGNORE INTO referral_region(referral_region_desc) SELECT DISTINCT referral_region_desc FROM t_medicare_1;

INSERT IGNORE INTO city_state_temp(city_state_name) SELECT DISTINCT city_state_name FROM t_medicare_1;
...
INSERT IGNORE INTO f_medicare (drg_desc, old_provider_id, provider_name, street, city_name, city_state_name, state_name, zip, referral_region_desc, total_discharges, avg_cov_charges, avg_total_payment, avg_med_payment, city_state_id, referral_region_id, drg_id, address_id, provider_id)
SELECT tm.drg_desc, tm.old_provider_id, tm.provider_name, tm.street, tm.city_name, tm.city_state_name, tm.state_name, tm.zip, tm.referral_region_desc, tm.total_discharges, tm.avg_cov_charges, tm.avg_total_payment, tm.avg_med_payment, cs.city_state_id, rr.referral_region_id, dr.drg_id, ad.address_id, pr.provider_id
...
-- Create provider-drg table (non temp table)
CREATE TABLE IF NOT EXISTS provider_drg
  (
    provider_drg_id INTEGER NOT NULL AUTO_INCREMENT UNIQUE,
    provider_id INTEGER NOT NULL,
...
INSERT IGNORE INTO city_state (city_state_name, state_name, city_name)
SELECT fm.city_state_name, fm.state_name, fm.city_name 
FROM f_medicare fm LEFT JOIN city_state_temp cs  ON fm.city_state_id = cs.city_state_id  WHERE IFNULL(fm.city_state_id, 0) = IFNULL(fm.city_state_id, 0)