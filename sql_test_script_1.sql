...
SELECT tm.drg_desc, tm.provider_name, tm.street, tm.city_state_name, tm.state_name, tm.city_name,
       ...
       tm.avg_total_payment, tm.total_discharges, tm.old_provider_id, cs.city_state_id, rr2.referral_region_id, dr.drg_id, ad.address_id, pr.provider_id
  FROM t_medicare_1 tm
       LEFT JOIN drg dr
              ON tm.drg_desc = dr.drg_desc
       LEFT JOIN provider_temp pr
              ON tm.old_provider_id = pr.old_provider_id
       LEFT JOIN referral_region_2 rr2
              ON tm.referral_region_desc = rr2.referral_region_desc
